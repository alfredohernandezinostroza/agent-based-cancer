...
class CanvasGridPrimary(VisualizationElement):
    ...
    def render(self, model):
        model.grid=model.grids[0]
        grid_state = defaultdict(list)
        for x in range(model.grids[0].width):
            for y in range(model.grids[0].height):
                cell_objects = model.grids[0].get_cell_list_contents([(x, y)])
...
class CanvasGridSecondary(VisualizationElement):
    """A CanvasGrid object uses a user-provided portrayal method to generate a
    portrayal for each object. A portrayal is a JSON-ready dictionary which
    tells the relevant JavaScript code (GridDraw.js) where to draw what shape.
    ...
    def render(self, model):
        model.grid=model.grids[1]
        grid_state = defaultdict(list)
        for x in range(model.grids[1].width):
            for y in range(model.grids[1].height):
                cell_objects = model.grids[1].get_cell_list_contents([(x, y)])
                for obj in cell_objects:
                    portrayal = self.portrayal_method(obj)
                    if portrayal:
                        portrayal["x"] = x
